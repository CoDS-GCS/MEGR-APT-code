...
then
  ...
  python -u src/${dataset_folder}/extract_rdf_subgraphs_${host}.py --database-name ${stardog_db} --parallel --extract-with-one-query --test-a-qg ${QG} --pg-name ${pg_name} --output-prx ${output_prx} > logs/${dataset_name}/${output_prx}/preprocessing_${stardog_db}_${QG}_in_${pg_name}_${date}.txt
fi
