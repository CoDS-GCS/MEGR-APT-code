...
preprocess_graph () {
    ...
    if [ ! -f ./dataset/${dataset}/experiments/${output_prx}/raw/torch_prediction/${QG}_in_${pg_name}.pt ]; then
        ...
        then
          ...
          python -u src/${dataset_folder}/extract_rdf_subgraphs_${host}.py --extract-with-one-query --output-prx ${output_prx} --test-a-qg ${QG} --pg-name ${pg_name} >> logs/${dataset_name}/${output_prx}/Evaluate_Per_Host/MEGRAPT_preprocessing_${host}_rdf_${date}.txt
        fi
    else
