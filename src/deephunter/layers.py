...
class AttentionModule(torch.nn.Module):
    ...
    def setup_weights(self):
        ...
        Defining weights.
        """
        #self.args.filters_3 + 32 , for attribute embedding concat 
    ...
    def forward(self, embedding):
        ...
        if self.args.sigmoid:
            activation_scores = torch.sigmoid(torch.mm(embedding, transformed_global.view(-1, 1)))
        else:
            #replace the sigmoid function with softmax and normalise weight into 1 
            activation_scores = torch.nn.functional.softmax(torch.mm(embedding, transformed_global.view(-1, 1)),dim=0)

        representation = torch.mm(torch.t(embedding), activation_scores)
