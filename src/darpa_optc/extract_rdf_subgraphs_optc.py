...
parser.add_argument('--ioc-file', nargs="?", help='Path of Query Graph IOCs file',
                    default="./dataset/darpa_optc/query_graphs_IOCs.json")
parser.add_argument('--dataset', nargs="?", help='Dataset name', default="darpa_optc")
...
parser.add_argument('--traverse-with-time', help='Consider timestamp while traversing', action="store_false",
                    default=True)
...
sparql_queries = {'Query_Suspicious_IP': """
    ...
    WHERE{
        ...
        ?_att <GRAPH_NAME>:src_ip|<GRAPH_NAME>:dest_ip ?ip .
...
""",
    ...
    WHERE{
        ?s rdf:type "file" .
        ?s <GRAPH_NAME>:attributes ?_att .
        ?_att <GRAPH_NAME>:file_paths ?paths .
...
""",
    ...
    WHERE{
        ?s rdf:type "process" .
        ?s <GRAPH_NAME>:attributes ?_att .
        ?_att <GRAPH_NAME>:image_paths|<GRAPH_NAME>:command_lines ?label .
        FILTER regex(?label, ?IOC ,'i') .
        ?s  <GRAPH_NAME>:uuid ?uuid .
    }
""",
...
def Traverse_rdf(params):
    ...
    if args.training:
        ...
        if args.traverse_with_time:
            try:
                csv_results = conn.select(graph_sparql_queries['Extract_Suspicious_Subgraph_withTime_v4'],
    ...
    try:
        if args.traverse_with_time:
            subgraphTriples['timestamp'] = subgraphTriples['timestamp'] 
    ...
    for index, row in nodes_df.iterrows():
        ...
        if row['type'] == 'shell':
            ...
            temp_df = pd.read_csv(io.BytesIO(csv_results))
            if temp_df.empty:
                attributes_df[row['uuid']] = {'type': 'shell'}
            else:
                temp_df['type'] = 'shell'
                attributes_df[row['uuid']] = temp_df.to_dict('records')[0]
